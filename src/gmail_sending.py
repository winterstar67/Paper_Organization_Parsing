...
Description:
    Phase 6: Enhanced email notification with Gmail API
...
# Register preferred browser for OAuth flow (WSL2 uses Chromium by default)
_browser_path = os.getenv(
    'BROWSER_PATH',
...
if os.path.exists(_browser_path):
    webbrowser.register(
        'preferred', None,
        webbrowser.BackgroundBrowser(_browser_path),
        preferred=True
    )

# Gmail API 스코프 - send-only 권한
SCOPES = ['https://www.googleapis.com/auth/gmail.send']
FORCE_CONSENT = os.getenv("GMAIL_OAUTH_FORCE_CONSENT", "").strip() == "1"

def create_directories() -> None:
    """
    필요한 디렉토리들을 생성합니다.

    Creates necessary directories for Gmail sending.

    Input: None

    Output: None (creates directories)
        - results/{date}/Phase_6: Results directory for email attachments
...
def authenticate_gmail_api() -> Optional[Any]:
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            ...
            # access_type='offline'으로 refresh token을 받고, 필요할 때만 동의 화면 표시
            if FORCE_CONSENT:
...
def load_latest_data() -> Optional[pd.DataFrame]:
    ...
    Example:
        >>> df = load_latest_data()
        데이터 로드 성공: results/organ_integrate.csv (25개 논문)
        >>> df is not None
        True
        >>> len(df)
        25
    """
    # Phase 5 (summary) 결과를 먼저 시도, 없으면 Phase 4 결과 사용
    files_to_try = [
        os.path.join(results_dir('Phase_5'), '5_abs_summary.csv'),
        os.path.join(results_dir('Phase_4'), 'organ_integrate.csv'),
    ]
...
def create_csv_attachment(df: pd.DataFrame) -> str:
    ...
    try:
        ...
        remaining_columns = [col for col in df_clean.columns if col not in priority_columns]
        df_clean = df_clean[[col for col in priority_columns if col in df_clean.columns] + remaining_columns]

        # 임시 파일로 저장
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        attachment_path = os.path.join(results_dir('Phase_6'), f'papers_for_email_{timestamp}.csv')
...
def load_collection_validation() -> Optional[Dict[str, Any]]:
    """
    Phase 1에서 저장한 수집 검증 리포트를 로드합니다.

    Input: None

    Output:
        - validation: Optional[Dict[str, Any]]
            - Ex: {"webpage_total": 217, "collected_count": 217, "validation_status": "PASS", "timestamp": "..."}
    """
    path = os.path.join(results_dir('Phase_1'), 'collection_validation.json')
    if os.path.exists(path):
        try:
            with open(path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            print(f"검증 리포트 로드 실패: {e}")
    return None

def create_email_content(df: pd.DataFrame, samples: List[Dict[str, Any]], start_date_str: str, end_date_str: str, validation: Optional[Dict[str, Any]] = None) -> str:
    """
    이메일 HTML 콘텐츠를 생성합니다.
    ...
    # Collection validation section
    if validation:
        v_status = validation.get('validation_status', 'UNKNOWN')
        v_webpage = validation.get('webpage_total', 'N/A')
        v_collected = validation.get('collected_count', 'N/A')
        v_time = validation.get('timestamp', 'N/A')
        ...
        if v_status == 'PASS':
            ...
            v_bg = '#d4edda'
            v_border = '#28a745'
            v_detail = f'웹페이지 표시: {v_webpage}개 = 실제 수집: {v_collected}개'
        elif v_status == 'FAIL':
            v_color = '#dc3545'
            v_icon = '⚠️'
            v_bg = '#f8d7da'
            v_border = '#dc3545'
            v_diff = (v_webpage or 0) - (v_collected or 0)
            v_detail = f'웹페이지 표시: {v_webpage}개 ≠ 실제 수집: {v_collected}개 (차이: {v_diff}개)'
        else:
            v_color = '#856404'
            v_icon = '❓'
            v_bg = '#fff3cd'
            v_border = '#856404'
            v_detail = f'웹페이지 총 논문 수 파싱 불가. 실제 수집: {v_collected}개'

        html_content += f"""
        <div style="padding: 15px; background-color: {v_bg}; border-left: 4px solid {v_border};
                    border-radius: 4px; margin-bottom: 25px;">
            <h3 style="color: {v_color}; margin-top: 0;">{v_icon} Phase 1 수집 검증: {v_status}</h3>
            <p style="margin: 5px 0;"><strong>검증 결과:</strong> {v_detail}</p>
            <p style="margin: 5px 0; font-size: 13px; color: #6c757d;">검증 시각: {v_time}</p>
        </div>
        """
...
def main() -> None:
    ...
    Main execution function for Phase 6: Gmail API paper report sending.
    ...
    Example:
        >>> main()
        ============================================================
        Phase 6: Gmail API를 통한 논문 보고서 발송
        ============================================================
        데이터 로드 성공: results/organ_integrate.csv (25개 논문)
        [OK] Gmail API 인증 성공
    ...
    """
    print("=" * 60)
    print("Phase 6: Gmail API를 통한 논문 보고서 발송")
    print("=" * 60)
    ...
    # Phase 1 수집 검증 리포트 로드
    validation = load_collection_validation()
    if validation:
        print(f"수집 검증 상태: {validation.get('validation_status', 'UNKNOWN')} "
              f"(웹페이지: {validation.get('webpage_total')}개, 수집: {validation.get('collected_count')}개)")
    else:
        print("수집 검증 리포트를 찾을 수 없습니다.")

    # 이메일 콘텐츠 생성
    html_content = create_email_content(df, samples, start_date_str, end_date_str, validation=validation)
