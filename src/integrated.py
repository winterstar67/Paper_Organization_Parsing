...
try:
    ...
    spec = importlib.util.spec_from_file_location("gmail_sending", "5_gmail_sending.py")
    gmail_sending = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gmail_sending)
except Exception as e:
    print(f"WARNING: 5_gmail_sending.py를 불러올 수 없습니다: {e}")
    gmail_sending = None
...
def main():
    ...
    print(f"[PHASE 2] WARNING: 이 단계는 243개 논문 * 5.2초 = 약 21분 소요됩니다")
    ...
    print(f"\nPhase 4: 기관 데이터 통합")
    if not run_script("4_organ_integrate.py"):
    ...
    if not run_script("5_gmail_sending.py"):
