...
- **입력**: ID_TABLE.csv 기반 지능형 날짜 범위 (최신 논문 다음날부터 어제까지, 최대 7일)
...
**주요 기능**:
- **데이터 기반 날짜 계산**: ID_TABLE.csv의 최신 Submitted 날짜 기준 지능형 검색 범위 설정
- **다중 주제 통합 검색**: cs.AI, cs.LG를 하나의 URL로 동시 검색
- **7일 제한**: 최대 검색 범위를 7일로 제한하여 효율성 확보
- **초기 실행 대응**: 빈 ID_TABLE일 때 어제부터 7일간 역순 논문 탐색
- **Submitted 날짜 표준화**: "8 Jan 2024" → "2024-01-08" 자동 변환
- **ID 테이블 관리**: ID, Paper_Title, Submitted 3컬럼 체계적 관리
- **중복 제거**: 제목 기반 중복 논문 자동 필터링
- **백업 시스템**: 논문 날짜 범위 기반 백업 파일 자동 생성
...
- **날짜 로직 혁신**: 공휴일/주말 알고리즘 제거, ID_TABLE.csv 기반 데이터 중심 수집 전략 도입 (2025-09-03)
- **ID 테이블 확장**: ID_TABLE.csv에 Submitted 컬럼 추가, YYYY-MM-DD 표준 형식으로 저장 (2025-09-03)
- **빈 데이터베이스 대응**: 초기 실행 시 어제부터 7일간 역순 탐색으로 논문 발견 (2025-09-03)
...
2. **데이터 기반 전략**: ID_TABLE.csv 기반 지능형 날짜 범위 계산 (2025-09-03)
   - ID_TABLE.csv에서 가장 최신 Submitted 날짜 확인
   - 다음날부터 어제(미국 시간)까지 검색
   - 최대 7일 범위 제한으로 과도한 검색 방지
   - ID_TABLE 비어있으면 어제부터 역순 검색(최대 7일)

**주요 개선사항**:
- ✅ **Submitted 날짜 표준화**: arXiv 형식을 YYYY-MM-DD로 변환하여 ID_TABLE.csv에 저장
- ✅ **지능형 검색 범위**: 기존 데이터 기반 최적 날짜 범위 자동 계산
- ✅ **초기 실행 대응**: 빈 ID_TABLE일 때 어제부터 7일간 역순 탐색
- ✅ **중복 방지**: 기존 논문 제목 기반 자동 필터링
- ✅ **공휴일/주말 로직 제거**: 복잡한 날짜 계산 대신 데이터 기반 접근

**현재 상태**: 데이터 기반 지능형 수집 시스템 완성
